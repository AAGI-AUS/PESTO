...
- par_ensemble:
  ...
  Numeric matrix `(n_par x n_real)`. Current parameter ensemble (each
  column is one realisation).

- obs_ensemble:

  Numeric matrix `(n_obs x n_real)`. Simulated observations from the
  current ensemble.
...
- weights:
  ...
  Numeric vector `(n_obs)`. Observation weights (typically
  `1 / sd(obs_noise)`).

- lambda:

  Numeric scalar. Marquardt damping parameter. `1.0` is the textbook GLM
  update; larger values dampen more aggressively.

## Value

A numeric matrix `(n_par x n_real)`: the parameter upgrade (add to
`par_ensemble` to get the next iterate).
...
1.  **Independent numerical validation.** PESTO's C++ kernel can be
    ...
    `tests/testthat/test-ensemble-solution-sign.R` and the paired-seed
    protocol in `inst/scripts/i2_paired_seed_check.R` both rely on
    this).
...
2.  **Self-contained `pestpp-ies` comparison.** Vignettes can compare
    ...
    reproducible on CRAN check farms and other binary-free environments.
...
[doi:10.1007/s10596-013-9351-5](https://doi.org/10.1007/s10596-013-9351-5)
...
[doi:10.1007/s10596-018-9731-y](https://doi.org/10.1007/s10596-018-9731-y)
